...
//-----------------------------------------------------------------------------
static CRITICAL_SECTION _tty_output_lock;

void tty_console_init() {
  InitializeCriticalSection(&_tty_output_lock);
}

void tty_enter_critical_section() {
  EnterCriticalSection(&_tty_output_lock);
}

void tty_leave_critical_section() {
  LeaveCriticalSection(&_tty_output_lock);
}
