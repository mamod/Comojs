...
function Socket (options){
    ...
    this.write_queue = [];
    var self = this;
    this.io_watcher = loop.handle_init(main_loop, function(h, events){
...
Socket.prototype.stream_io = function(events){
    ...
    if (events & _write_mask) {
        ...
        if (!this.write_queue.length){
            this.write_queue.length = 0;
            loop.io_stop(this.io_watcher, loop.POLLOUT);
            if (this.stream_shutdown) {
                loop.handle_close(this.io_watcher);
                if ( this.fd === -1 ) return;
...
Socket.prototype.stream_eof = function(buf){
    this.eof = true;
    loop.io_remove(this.io_watcher, loop.POLLIN);
    if (!loop.io_active(this.io_watcher, loop.POLLOUT)){
        this.close();
    }
...
Socket.prototype.close = function(buf){
    //close on next tick to flush all unwritten requests
    loop.io_start(this.io_watcher, this.fd, POLLWRITE);
};
...
Socket.prototype.connect = function(ip, port, cb){
    ...
    loop.io_start(this.io_watcher, this.fd, loop.POLLOUT | loop.POLLERR);
};
...
Socket.prototype.listen = function(backlog, cb){
    ...
    this.handle = loop.handle_init(main_loop, function(h, events){
        self.server_io(events);
    });

    loop.io_start(this.handle, this.fd, loop.POLLIN);
};
...
Socket.prototype.read_start = function(cb){
    ...
    loop.io_start(this.io_watcher, this.fd, POLLREAD);
};
...
Socket.prototype.write = function(buf){
    ...
    if (!loop.io_active(this.io_watcher, loop.POLLOUT)){
        loop.io_start(this.io_watcher, sockfd, POLLWRITE);
    }
};
